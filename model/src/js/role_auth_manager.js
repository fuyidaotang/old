var roleAuthManagerVue;
function initialize() {
    clearForm()
    roleAuthManagerVue = new Vue({
        el:".role_auth_manager",
        data:{
            roleId:1,
            authList:[],
            roleList:[]
        ...
        methods:{
            ...
            changeAuth:function(permissionId,action){
                var para = {};
                para.roleId = this.roleId;
                para.permissionId = permissionId;
                para.action = action?"add":"del";
                console.log(para);
                $.ajax({
                    type:"post",
                    data:para,
                    url:"/tvmanager/sys/changeRolePermission",
...
function getAuthList(){
    var para = {};
    para.roleId = roleAuthManagerVue.roleId;
    $.ajax({
        type:"get",
        data:para,
        url:"/tvmanager/sys/roleAuthPermission",
        success:function(data){
            data = JSON.parse(data);
            console.log(data);
            if(data.code = 20000){
                ...
                roleAuthManagerVue.authList = authList;
            }else if(data.code == 20004){
                alert("登录失效,请重新登录");
                sessionStorage.removeItem("loginInfo");
                location.href="login.html";
...
function getRoleList(){
    $.ajax({
        ...
        success:function(data){
            ...
            if(data.resp){
                roleAuthManagerVue.roleList = data.resp.roleInfo;
                console.log(data.resp.roleInfo);
            }
        }
    });
}
...
function menuInit(menuInfo){
    ...
    for(var i=0;i<menuInfo.length;i++){
        ...
        if(menuInfo[i].parentId == 0){
            ...
            menuItem.permissionEx = menuInfo[i].permissionEx;
            menuItem.menuId = menuInfo[i].menuId;
